...
<script type="text/javascript">
	...
	function goSearch(){
		...
		frm.method = "GET";
		frm.action = "<%= ctxPath%>/edms/mydoc/waitlist.bts";
		frm.submit();
...
<div class="edmsDiv">
	...
	<div id="mywaitlist">
		...
		<%-- 결재대기 목록이 없을 때 시작 --%>
		<c:if test="${empty requestScope.mywaitlist}">
		...
		<%-- 결재대기 목록이 있을 때 종료 --%>
		<c:if test="${not empty requestScope.mywaitlist}">
		<table class="table table-sm table-hover table-light edmsTable ellipsisTable">
			...
			<tbody>
				<c:forEach var="wait" items="${requestScope.mywaitlist}" varStatus="status">
