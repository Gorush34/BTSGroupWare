...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$("select[name=fk_department_id]").val((("${requestScope.empList.fk_department_id}" == '') ? "" : "${requestScope.empList.fk_department_id}")).prop("selected", true); 
		$("select[name=fk_rank_id]").val((("${requestScope.empList.fk_rank_id}" == '') ? "" : "${requestScope.empList.fk_rank_id}")).prop("selected", true); 
		$("input#postcode").val("${requestScope.empList.postal}");
		
		$("select[name=num1]").val((("${requestScope.empList.num1}" == '') ? "" : "${requestScope.empList.num1}")).prop("selected", true); 
		$("input[name=num2]").val( ("${requestScope.empList.num2}" == '') ? "" : "${requestScope.empList.num2}" ); 
		$("input[name=num3]").val( ("${requestScope.empList.num3}" == '') ? "" : "${requestScope.empList.num3}" ); 
		
		$("input[name=hp2]").val( ("${requestScope.empList.hp2}" == '') ? "" : "${requestScope.empList.hp2}" ); 
		$("input[name=hp3]").val( ("${requestScope.empList.hp3}" == '') ? "" : "${requestScope.empList.hp3}" ); 
	...
	function updateHireOne() {
		
		const frm = document.updateFrm;
		frm.action = "<%= ctxPath%>/emp/updateHireOne.bts";
		frm.method = "POST";
		frm.submit();
...
<div id="tbl_regEmp">
<c:set var="emp" value="${requestScope.empList}"/>
	...
	<div style="padding-left: 200px;">
		<button class="btn btn-info" id="btn_register" style="" onclick="goUpdate();">저장</button>
		<button class="btn btn-warning" id="btn_register" style="margin-left: 10px;  border:solid 1px gray; color: white;" onclick="updateHireOne();">재직상태 변경</button>
		<button class="btn btn-primary" id="btn_list" style="margin-left: 10px;" onclick="javascript:location.href='<%= request.getContextPath()%>/emp/showAllEmp.bts'">목록</button>
	</div>
